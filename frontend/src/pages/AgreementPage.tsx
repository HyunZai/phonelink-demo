...
const AgreementPage: React.FC = () => {
  ...
  const handleAllAgree = () => {
    ...
    setAgreements({
      agreePrivacyUse: newValue,
      agreeAgeOver14: newValue,
      agreeTerms: newValue,
    });
  };

  const handleIndividualAgree = (field: keyof typeof agreements) => {
    setAgreements((prev) => ({
      ...prev,
      [field]: !prev[field],
    }));
  ...
  const agreementItems = [
    {
      id: "agreeAgeOver14" as const,
      label: "만 14세 이상 확인",
      required: true,
      content: `PhoneLink는 만 14세 미만 아동은 가입할 수 없습니다.`,
    },
    {
      id: "agreePrivacyUse" as const,
      label: "개인정보 수집 및 이용 동의",
      required: true,
      content: `PhoneLink(이하 "회사")는 개인정보보호법에 따라 이용자의 개인정보 보호 및 권익을 보호하고자 다음과 같이 개인정보처리방침을 수립·공개합니다.
...
【개인정보 처리의 위탁】
회사는 현재 개인정보 처리 위탁을 하지 않습니다. 향후 위탁이 필요한 경우, 위탁받는 자와 위탁하는 업무의 내용에 대해 사전에 공지하고 필요한 경우 사전 동의를 받겠습니다.

【이용자의 권리·의무 및 그 행사 방법】
이용자는 언제든지 개인정보 열람·정정·삭제·처리정지 요구 등의 권리를 행사할 수 있으며, 회사는 이에 대해 지체 없이 조치하겠습니다. 다만, 다음의 경우에는 예외로 합니다.
...
제1조 (목적)
이 약관은 PhoneLink(이하 "회사" 또는 "사이트")가 제공하는 온라인 서비스(이하 "서비스")의 이용과 관련하여 회사와 이용자 간의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.

제2조 (정의)
① "서비스"란 회사가 제공하는 스마트폰 견적 비교, 매장 정보 제공, 커뮤니티 서비스 등 모든 온라인 서비스를 의미합니다.
...
제3조 (약관의 게시와 개정)
① 회사는 이 약관의 내용을 이용자가 쉽게 알 수 있도록 서비스 초기 화면(전면)에 게시합니다.
② 회사는 필요한 경우 관련 법령을 위배하지 않는 범위에서 이 약관을 개정할 수 있습니다.
③ 회사가 약관을 개정할 경우에는 적용일자 및 개정사유를 명시하여 현행약관과 함께 서비스의 초기 화면에 그 적용일자 7일 이전부터 적용일자 전일까지 공지합니다. 다만, 이용자에게 불리하게 약관 내용을 변경하는 경우에는 최소한 30일 이상의 사전 유예기간을 두고 공지합니다.
...
② 회사는 필요한 경우 서비스의 내용을 추가 또는 변경할 수 있습니다. 다만, 이 경우에는 추가 또는 변경내용을 즉시 공지합니다.
...
② 회사는 불특정다수 회원에 대한 통지의 경우 1주일 이상 회사의 게시판에 게시함으로서 개별 통지에 갈음할 수 있습니다.

제9조 (개인정보보호)
① 회사는 이용자의 개인정보 수집 시 서비스 제공을 위하여 필요한 범위에서 최소한의 개인정보를 수집합니다.
② 회사는 회원가입 시 서비스 제공에 필요한 최소한의 정보만을 수집하며, 회원은 개인정보의 보호 및 관리에 대한 개별 동의를 해야 합니다.
③ 회사는 이용자로부터 수집한 개인정보를 목적 외의 용도로 이용하거나 제3자에게 제공하지 않으며, 관련 법령에 따라 관리·보호합니다.
④ 회사의 개인정보보호책임자 및 연락처는 개인정보처리방침에서 확인하실 수 있습니다.

제10조 (회사의 의무)
① 회사는 법령과 이 약관이 금지하거나 공서양속에 반하는 행위를 하지 않으며, 지속적이고 안정적으로 서비스를 제공하는데 최선을 다합니다.
② 회사는 이용자가 안전하게 서비스를 이용할 수 있도록 개인정보(신용정보 포함) 보호를 위한 보안시스템을 갖추며, 개인정보처리방침을 공시하고 준수합니다.
③ 회사는 서비스와 관련하여 이용자로부터 제기된 의견이나 불만이 정당하다고 인정할 경우에는 이를 처리하여야 합니다.
...
② 회사와 이용자 간에 발생한 전자상거래 분쟁에 관한 소송은 제소 당시의 이용자의 주소에 의하고, 주소가 없는 경우에는 거소를 관할하는 지방법원의 전속관할로 합니다. 다만, 제소 당시 이용자의 주소 또는 거소가 명확하지 아니한 경우의 관할법원은 민사소송법에 따라 정합니다.
③ 회사와 이용자 간에 발생한 분쟁은 한국법을 적용합니다.
